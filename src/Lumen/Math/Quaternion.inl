...
constexpr Quaternion Quaternion::Inverted() const
{
...
constexpr Matrix4 Quaternion::ToMatrix() const
{
...
constexpr Vector3 Quaternion::ToEuler() const
{
    Vector3 result;
...
constexpr Quaternion Quaternion::FromAxisAngle(const Vector3 &axis, float angle)
{
...
constexpr Quaternion Quaternion::FromEuler(float pitch, float yaw, float roll)
{
...
constexpr Quaternion Quaternion::FromMatrix(const Matrix4 &mat)
{
    Quaternion result;
...
constexpr Quaternion Quaternion::FromVector3ToVector3(const Vector3 &from,
                                                      const Vector3 &to)
{
...
constexpr Quaternion Quaternion::Lerp(const Quaternion &q1, const Quaternion &q2,
                                      float amount)
{
    return q1 + (q2 - q1) * Math::Clamp(amount, 0.0f, 1.0f);
}

constexpr Quaternion Quaternion::Nlerp(const Quaternion &q1, const Quaternion &q2,
                                       float amount)
{
    return Lerp(q1, q2, amount).Normalized();
}

constexpr Quaternion Quaternion::Slerp(const Quaternion &q1, const Quaternion &q2,
                                       float amount)
{
    Quaternion result;
    ...
    else
    {
        ...
        if (Math::Abs(sinHalfTheta) < Math::Epsilon)
        {
...
constexpr Quaternion operator+(const Quaternion &lhs, float value)
{
    return {
        lhs.x + value,
        lhs.y + value,
...
constexpr Quaternion operator*(const Quaternion &lhs, float scalar)
{
    return {
        lhs.x * scalar,
        lhs.y * scalar,
...
constexpr Quaternion operator*(float scalar, const Quaternion &rhs)
{
    return rhs * scalar;
}
...
constexpr Quaternion &operator+=(Quaternion &lhs, const Quaternion &rhs)
{
    lhs = lhs + rhs;
    return lhs;
}
...
constexpr Quaternion &operator*=(Quaternion &lhs, float scalar)
{
    lhs = lhs * scalar;
    return lhs;
}

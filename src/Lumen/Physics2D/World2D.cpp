...
World2D::World2D(const WorldDef2D &worldDef)
{
...
Body2DEvents World2D::GetBodyEvents() const
{
...
Contact2DEvents World2D::GetContactEvents() const
{
...
WorldCounters2D World2D::GetCounters() const
{
...
Vector2 World2D::GetGravity() const
{
    b2Vec2 vec = b2World_GetGravity(m_WorldID);
    return {vec.x, vec.y};
}

WorldProfile2D World2D::GetProfile() const
{
...
Sensor2DEvents World2D::GetSensorEvents() const
{
    ...
    for (int i = 0; i < events.beginCount; i++)
    {
        SensorBeginTouch2DEvent sensorEvent = {events.beginEvents[i].sensorShapeId,
                                               events.beginEvents[i].visitorShapeId};
        sensorEvents.BeginEvents.push_back(sensorEvent);
    }
    for (int i = 0; i < events.endCount; i++)
    {
        SensorEndTouch2DEvent sensorEvent = {events.beginEvents[i].sensorShapeId,
                                             events.beginEvents[i].visitorShapeId};
...
Body2D World2D::CreateBody(const BodyDef2D &bodyDef)
{
    b2BodyDef def = bodyDef;
    b2BodyId bodyID = b2CreateBody(m_WorldID, &def);
    return {bodyID};
}

void World2D::DestroyBody(const Body2D &body)
{
    b2DestroyBody(body.GetBodyID());
}

void World2D::Update(float timeStep, int subStepCount)
{
